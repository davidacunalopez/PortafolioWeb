...
function EventCard({ item, photo }: { item: EventItem; photo: string }) {
  return (
    <figure className="w-[78vw] max-w-[440px] shrink-0 snap-center overflow-hidden rounded-2xl border border-line bg-surface">
        ...
        <div className="sticky top-0 flex h-screen flex-col justify-center overflow-hidden">
          ...
          >
            {t.events.items.map((item, i) => (
              <EventCard key={item.title} item={item} photo={photos[i]} />
            ))}
          </motion.div>
        </div>
      </section>
